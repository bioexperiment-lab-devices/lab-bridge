...
- **Devices not detected** — switch to the **Devices** tab and click **Rediscover**. SerialHop re-scans serial ports and picks up any newly plugged-in device without restarting the service. If some boards consistently miss discovery, raise **discovery → Post-open settle** in the Config tab; see [the config reference](/docs/operator/config#discovery).
